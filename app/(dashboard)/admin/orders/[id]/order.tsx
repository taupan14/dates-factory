  ...
  useEffect(() => {
    ...
    const fetchOrderProducts = async () => {
      const response = await fetch(
        ...
        `http://localhost:3001/api/order-product/${id}`
      );
      ...
      <div className="flex flex-col gap-y-7 xl:ml-5 w-full max-xl:px-5">
        ...
        <div>
          {orderProducts?.map((product) => (
            <div className="flex items-center gap-x-4 mt-2" key={product?.id}>
              <Image
                src={
              ...
              <div>
                ...
                <p>
                  IDR {product?.product?.price}K * {product?.quantity} items
                </p>
              </div>
            </div>
          ))}
          <div className="flex flex-col gap-y-2 mt-10">
            <p className="text-2xl">Subtotal: IDR {order?.total}K</p>
            <p className="text-2xl">Tax 20%: IDR {order?.total / 5}K</p>
            <p className="text-2xl">Shipping: IDR 5K</p>
            <p className="text-3xl font-semibold">
              Total: IDR {order?.total + order?.total / 5 + 5}K
            </p>
          </div>
